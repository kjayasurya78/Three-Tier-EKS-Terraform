...
flowchart TD
    ...
    subgraph AWS["☁️ AWS ap-south-1"]
      ...
      subgraph VPC["VPC  10.0.0.0/16"]
        ...
        subgraph PUB["Public Subnets (AZ-a · AZ-b)"]
          ...
          subgraph JEC2["Jenkins EC2  t3.medium"]
            JEN["Jenkins\n:8080"]
            SQ["SonarQube\nDocker :9000"]
          end
        end
        ...
        subgraph PRIV["Private Subnets (AZ-a · AZ-b)"]
          ...
          subgraph EKS["EKS Cluster — three-tier-cluster\n2 × t3.medium nodes (autoscale → 4)"]
            ...
            subgraph ARGOCD["namespace: argocd"]
              ARGO["ArgoCD Server\nLoadBalancer :80\npoll GitHub every 3 min"]
            end

            subgraph MON["namespace: monitoring"]
              PROM["Prometheus\nkube-prometheus-stack\nscrape :9090"]
              GRAF["Grafana\nLoadBalancer :80\ndashboards: 6417 · 6336 · 1860 · 9614"]
            end

            subgraph HMSHOP["namespace: hm-shop"]

              ING["Ingress  hm-shop-ingress\nIngressClass: alb"]

              subgraph FE_GROUP["Frontend"]
                FE_SA["ServiceAccount\nfrontend-sa\nIRSA → frontend-ecr-role"]
                FE_DEP["Deployment\nReact 18 / Nginx\nport 8080 · replicas: 2"]
                FE_SVC["Service\nfrontend-service\nClusterIP :80"]
                FE_HPA["HPA\nmin 2 → max 5\nCPU 70% · MEM 80%"]
              end

              subgraph BE_GROUP["Backend"]
                BE_SA["ServiceAccount\nbackend-sa\nIRSA → backend-ecr-role"]
                BE_DEP["Deployment\nNode.js 18 / Express\nport 5000 · replicas: 2\nrunAsUser: 1000"]
                BE_SVC["Service\nbackend-service\nClusterIP :5000"]
                BE_HPA["HPA\nmin 2 → max 5\nCPU 70% · MEM 80%"]
              ...
              subgraph DB_GROUP["Database"]
                ...
                PG_SVC["Service\npostgres-service\nClusterIP :5432"]
                PG_HPA["HPA\nmin 1 → max 2"]
                PG_PVC["PVC  postgres-pvc\n10Gi  RWO"]
                PG_SEC["Secret  postgres-secret\nUSER · PASSWORD · DB"]
                PG_CFG["ConfigMap\ninit-configmap\nSQL seed data"]
                SC["StorageClass  hm-ebs-gp2\nebs.csi.aws.com\nRetain · WaitForFirstConsumer"]
              end
    ...
    GH -->|"webhook  POST :8080/github-webhook/"| JEN
    JEN -->|"Stage 1 — sonar-scanner"| SQ
    SQ -->|"Quality Gate callback\nPOST :8080/sonarqube-webhook/"| JEN
    JEN -->|"Stage 2 — trivy fs scan"| JEN
    JEN -->|"Stage 3 — docker build\nmulti-stage · non-root"| JEN
    JEN -->|"Stage 4 — ecr get-login-password\ndocker push  :tag"| ECR_FE
    JEN -->|"Stage 4 — docker push  :tag"| ECR_BE
    JEN -->|"Stage 5 — trivy image scan\n(ECR pull)"| ECR_FE
    JEN -->|"Stage 6 — git commit\nUpdate image tags [skip ci]"| GH

    %% ── GitOps ───────────────────────────────────────────────────────────────
    GH -->|"poll  k8s_manifests/\nevery 3 min"| ARGO
    ARGO -->|"kubectl apply\nk8s_manifests/"| HMSHOP

    %% ── Image pulls via IRSA ─────────────────────────────────────────────────
    ECR_FE -->|"pull image\nsts:AssumeRoleWithWebIdentity"| FE_DEP
    ECR_BE -->|"pull image\nsts:AssumeRoleWithWebIdentity"| BE_DEP
    OIDC -.->|"IRSA binding"| FE_SA
    OIDC -.->|"IRSA binding"| BE_SA
    OIDC -.->|"IRSA binding"| ALBCTRL
    OIDC -.->|"IRSA binding"| CA

    %% ── Ingress & routing ────────────────────────────────────────────────────
    ALBCTRL -->|"provisions"| ALB
    ALB -->|"/"| ING
    ING -->|"/ → ClusterIP :80"| FE_SVC
    ING -->|"/api  /metrics → ClusterIP :5000"| BE_SVC
    FE_SVC --> FE_DEP
    BE_SVC --> BE_DEP

    %% ── App-tier wiring ──────────────────────────────────────────────────────
    BE_DEP -->|"TCP :5432"| PG_SVC
    PG_SVC --> PG_DEP
    BE_DEP -.-|"env from"| BE_SEC
    PG_DEP -.-|"env from"| PG_SEC
    PG_DEP -.-|"mount"| PG_CFG
    FE_DEP -.-|"bound to"| FE_SA
    BE_DEP -.-|"bound to"| BE_SA

    %% ── Storage ──────────────────────────────────────────────────────────────
    PG_DEP -->|"volumeMount"| PG_PVC
    PG_PVC -->|"storageClassName: hm-ebs-gp2"| SC
    SC -->|"dynamic provision"| EBS
    EBSCSI -->|"manages lifecycle"| EBS
    ...
    %% ── Networking ───────────────────────────────────────────────────────────
    IGW --- NAT
    NAT -.->|"outbound internet\nfor private subnets"| PRIV

    %% ── User access ──────────────────────────────────────────────────────────
    USER -->|"HTTP :80"| ALB
    DEVOPS -->|"HTTP :80  LoadBalancer"| GRAF
    DEVOPS -->|"HTTP :80  LoadBalancer"| ARGO
```
